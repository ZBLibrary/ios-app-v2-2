...
@objc protocol MyDeviceMainControllerDelegate
{
...
class MyDeviceMainController: UIViewController,CustomNoDeviceViewDelegate,CustomOCCircleViewDelegate,OznerDeviceDelegate,UIScrollViewDelegate,UIAlertViewDelegate{
    ...
    //台式空气净化器
    //尾部视图
    var smallFooterView:smallFooterViewXib!
    var smallStateView:smallStateXib!
    
    //立式空气净化器
    var bigFooterViews=[bigFooterViewXib]()
    var bigStateView:bigStateXib!
    var bigModelBgView:UIView!//auto点击后视图
    var tmpbigmodel:bigautoModelView!
    ...
    var myCircleView:CustomOCCircleView?
    //侧滑黑背景
    var leftSlideBG_gray:UIView!
    //净水器
    var WaterPurfHeadView:WaterPurifierHeadCell?
    //是否隐藏tableBar
    var m_bIsHideTableBar = false
    //当前选中的device
    var myCurrentDevice:OznerDevice?
    //探头
    var myTanTouBgView:MyDeviceCustomFitstView?
    //探头水TDS
    var tanTouDataSource:NSArray?
    //净水器
    var waterPurFooter:WaterPurFooterCell!
    //排名
    var currentRank:Int?
    ...
    // 连接状态
    var LoadingView:LoadingXib!
    //新手引导透明view
    var myAlphaBgView:UIView?
    //下面的view
    var noDeviceBottomView:CustomNoDeviceView!
...
//        }
    ...
    var defaultFooterView:Main_FooterView_zb!
    //Cup页面视图
    ...
    func loadWhitchView(type:String)
    {
        ...
        case CupManager.isCup(type):
            ...
            //头部视图
            let circleView = CustomOCCircleView.init(frame: CGRectMake((Screen_Width/375.0)*32, (Screen_Width/375.0)*57, Screen_Width-2*(Screen_Width/375.0)*32, CGFloat((Screen_Width-2*(Screen_Width/375.0)*32)/2)+20*(Screen_Hight/667.0)),tdsValue:0,beatValue:Int32(self.currentDefeat!),rankValue:0)
        ...
        case type == TDSPAN_TYPE:
            ...
            //头部视图
            let circleView = CustomOCCircleView.init(frame: CGRectMake((Screen_Width/375.0)*32, (Screen_Width/375.0)*100, Screen_Width-2*(Screen_Width/375.0)*32, CGFloat((Screen_Width-2*(Screen_Width/375.0)*32)/2)+20*(Screen_Hight/667.0)),tdsValue:0,beatValue:Int32(self.currentDefeat!),rankValue:0)
            self.myCircleView = circleView
            circleView.delegate = self
            circleView.backgroundColor = UIColor.clearColor()
            deviceHeadView.addSubview(circleView)
        ...
        case TapManager.isTap(type):
            ...
            //头部视图
            let circleView = CustomOCCircleView.init(frame: CGRectMake((Screen_Width/375.0)*32, (Screen_Width/375.0)*30, Screen_Width-2*(Screen_Width/375.0)*32, CGFloat((Screen_Width-2*(Screen_Width/375.0)*32)/2)+20*(Screen_Hight/667.0)),tdsValue:0,beatValue:Int32(self.currentDefeat!),rankValue:0)
            self.myCircleView = circleView
            circleView.delegate = self
            circleView.backgroundColor = UIColor.clearColor()
            deviceHeadView.addSubview(circleView)
...
//    }
    ...
    func obtainTanTouTDS()
    {
        ...
        {
            self.tanTouDataSource = CustomCommFunction.customCommFunctionInstance().sortTapListMaxTime(self.tanTouDataSource! as [AnyObject])
        }
...
//                    }
    ...
    */
    ...
    func loadAirCleanerView()
    {
        ...
        headView=NSBundle.mainBundle().loadNibNamed("headViewView", owner: nil, options: nil).last as! headViewView
        ...
        updateOutAirData()
        //室内空气提醒
        IAW_TempView=NSBundle.mainBundle().loadNibNamed("indoorAirWarn_Air", owner: nil, options: nil).last as! indoorAirWarn_Air
        ...
        else
        {
            ...
            tmpbigmodel=NSBundle.mainBundle().loadNibNamed("bigautoModelView", owner: nil, options: nil).last as! bigautoModelView
            ...
            for i in 1...3 //7
            {
                let tmpbigFooter=NSBundle.mainBundle().loadNibNamed("bigFooterViewXib", owner: nil, options: nil).last as! bigFooterViewXib
                //if i<=4
                //{
