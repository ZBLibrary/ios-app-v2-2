...
+ (NSMutableAttributedString *)emotionStrWithString:(NSString *)text
{
    ...
    NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithData:[text dataUsingEncoding:NSUnicodeStringEncoding] options:@{ NSDocumentTypeDocumentAttribute: NSHTMLTextDocumentType } documentAttributes:nil error:nil];
    //2、通过正则表达式来匹配字符串
    ...
    for (int i = (int)imageArray.count -1; i >= 0; i--) {
        ...
        //进行替换
        NSLog(@"%@",imageArray[i][@"image"]);
        if (attributeString!=nil&&imageArray.count>0) {
            NSRange range;
