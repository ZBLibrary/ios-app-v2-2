...
- (void)showViewWithType:(XMFunctionViewShowType)showType{
    ...
    if (showType == XMFunctionViewShowNothing) {
        [[CustomTabBarView sharedCustomTabBar] showAllMyTabBar];//CustomTabBarView.sharedCustomTabBar().showAllMyTabBar()
    }
    else
    {
    ...
    switch (showType) {
        ...
        case XMFunctionViewShowVoice:
        {
            ...
            [self setFrame:CGRectMake(0, self.screenHeight - kMinHeight-65, self.frame.size.width, kMinHeight) animated:NO];
            //[self.textView resignFirstResponder];
        }
            break;
        ...
        case XMFunctionViewShowFace:
            self.inputText = self.textView.text;
            [self setFrame:CGRectMake(0, self.screenHeight - kFunctionViewHeight - self.textView.frame.size.height - 10-65, self.frame.size.width, self.textView.frame.size.height + 10) animated:NO];
            [self.textView resignFirstResponder];
            [self textViewDidChange:self.textView];
            //UITabBar.hidden=true
            break;
...
- (void)sendImageMessage:(UIImage *)image{
    if (self.delegate && [self.delegate respondsToSelector:@selector(chatBar:sendPictures:)]) {
        ...
        [self.delegate chatBar:self sendPictures:@[tmpimage]];
    }
}
