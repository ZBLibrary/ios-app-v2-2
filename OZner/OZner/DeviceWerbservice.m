...
- (ASIFormDataRequest*)GetMachineType:(NSString*)type returnBlock:(void(^)(NSString* IsShowScan,NSString* MachineType,NSString* Attr,NSString* BuyUrl,NSString* days, StatusManager* status))handle{
    ...
    return [WebAssistant execNormalkRequest:entrance bodyBlock:^(NSDictionary *dicBody, StatusManager *status) {
        ...
        handle([tmpdic objectForKey:@"boolshow"],[tmpdic objectForKey:@"MachineType"],[tmpdic objectForKey:@"Attr"],[tmpdic objectForKey:@"buylinkurl"],[tmpdic objectForKey:@"days"],status);
    } failedBlock:^(StatusManager *status) {
        handle(nil,nil,nil,nil,nil,status);
    }];
}
