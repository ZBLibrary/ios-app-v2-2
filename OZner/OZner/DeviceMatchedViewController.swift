...
class DeviceMatchedViewController: SwiftFatherViewController,iCarouselDataSource,iCarouselDelegate,MatchFinishedViewDelegate,MatchTanTouFinisedViewDelegate,JinShuiqiWIFIControllerDelegate,UIAlertViewDelegate,UITextFieldDelegate,OznerManagerDelegate {
    ...
    func canMatchDevices()->NSArray
    {
        ...
        for var i = 0; i < arr.count;i++
        {
            ...
            {
                if(self.deviceCuttentType == 0)
                {
                    if (CupManager.isCup(deviceIo.type) == true)
                    {
                        muArr .addObject(deviceIo)
                    }
                    
                }
                else if(self.deviceCuttentType == 1)
                {
                    if (TapManager.isTap(deviceIo.type) == true)
                    {
                        muArr .addObject(deviceIo)
                    }
                }
                ...
                {
                    //判断是否是净水器
                    if WaterPurifierManager.isWaterPurifier(deviceIo.type) == true
                    {
                        muArr .addObject(deviceIo)
                    }
                }
                ...
                {
                    //判断是否是台式空气净化器
                    if (AirPurifierManager.isBluetoothAirPurifier(deviceIo.type) == true)
                    {
                        muArr .addObject(deviceIo)
                    }
                }
                ...
                {
                    //判断是否是立式空气净化器
                    if (AirPurifierManager.isMXChipAirPurifier(deviceIo.type) == true)
                    {
                        muArr .addObject(deviceIo)
                    }
                }
    ...
    func layOUtView()
    {
        ...
        else if(self.deviceCuttentType == 3)
        {
            ...
            animationImgView.image=UIImage(named: "icon_peidui_complete_smallair.png")//图片缺少
        }
        else if(self.deviceCuttentType == 4)
        {
        ...
        {
            self.finishedBgView?.hidden = true
            self.tanTouFinishedView?.hidden = false
