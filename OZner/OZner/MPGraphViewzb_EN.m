...
#import "MPGraphViewzb_EN.h"
#import "UIBezierPath+curved.h"


@implementation MPGraphViewzb_EN
...
- (id)initWithFrame:(CGRect)frame timeType:(int)timeType cellType:(int)cellType title1:(NSString*)title1 title2:(NSString*)title2 title3:(NSString*)title3
{
    ...
    if (self)
    {
        ...
        if (timeType != 28) {//28 表示水探头页面
            ...
            NSString* content = [NSString stringWithFormat:@"%@(0%%)",self.title1];
            ...
            circleView.layer.masksToBounds = YES;
            circleView.layer.cornerRadius = circleView.frame.size.width/2;
            circleView.layer.borderColor = cellType==0 ? [UIColor colorWithRed:128.0/255 green:94.0/255 blue:230.0/255 alpha:1.0].CGColor:[UIColor colorWithRed:242.0/255 green:134.0/255 blue:82.0/255 alpha:1.0].CGColor;
            circleView.layer.borderWidth = 1;
            [self addSubview:circleView];
            
            content = [NSString stringWithFormat:@"%@(0%%)",self.title2];
