...
class setAirViewController: UIViewController,UIAlertViewDelegate {

    var myCurrentDevice:OznerDevice?
    let plistName:String=get_CurrSelectEquip()==4 ? "setSmallAir":"setBigAir"
    ...
    //返回
    func back(){
        let alert=UIAlertView(title: "", message: "是否保存？", delegate: self, cancelButtonTitle: "否", otherButtonTitles: "是")
        alert.tag=1
        alert.show()
