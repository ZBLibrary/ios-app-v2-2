...
+ (NSString*)logPath
{
    NSString* documentPath = [self documentsFolderPath];
    ...
    NSString* filePath = [NSString stringWithFormat:@"%@/Log/%@/%@/%@/%@",documentPath,[arr objectAtIndex:0],[arr objectAtIndex:1],[dateArr objectAtIndex:0],[dateArr objectAtIndex:1]];
    
    NSFileManager* fileManager = [NSFileManager defaultManager];
    if(![fileManager fileExistsAtPath:[NSString stringWithFormat:@"%@/log.text",filePath]])
    {
        [fileManager createDirectoryAtPath:filePath withIntermediateDirectories:YES attributes:nil error:nil];
        [fileManager createFileAtPath:[NSString stringWithFormat:@"%@/log.text",filePath] contents:NULL attributes:nil];
    }
    
    return [NSString stringWithFormat:@"%@/log.text",filePath];
}
...
+ (NSString* )myPicCacheFilePath:(NSString* )picURL
{
    ...
    NSArray* arr = [picURL componentsSeparatedByString:@"/"];
...
+ (NSString *)readExtendedAttributeWithPath:(NSString *)path key:(NSString *)key
{
    ...
    return [[[NSString alloc] initWithData:[extendedAttributes objectForKey:key] encoding:NSUTF8StringEncoding] autorelease];
}
...
+ (NSMutableDictionary *)readAllExtendedAttributesWithPath:(NSString *)path
{
    ...
    for (id key in extendedAttributes) {
        [dict setObject:[[[NSString alloc] initWithData:[extendedAttributes objectForKey:key] encoding:NSUTF8StringEncoding] autorelease] forKey:key];
    }
...
+ (NSString *)transformString:(NSString *)string
{
    ...
    //html &
    range=[result rangeOfString:@"&" options:NSRegularExpressionSearch range:NSMakeRange(0, [result length])];
    if(range.length > 0)
    {
...
//	}
    ...
    if(range.length > 0)
    {
        [result insertString:@" " atIndex:range.location+range.length];
    }
    while (range.length>0)
	{
        NSString* subStr = [result substringWithRange:range];
        subStr = [faceDict objectForKey:subStr];
        replace=[NSString stringWithFormat:@"<img src='%@' width='16' height='16' />",subStr];
        [result replaceCharactersInRange:range withString:replace];
        range=NSMakeRange(range.location+replace.length, [result length]-(range.location+replace.length));
		range=[result rangeOfString:regexStr options:NSRegularExpressionSearch range:range];
	}
...
+ (UIView *)horizontalSolidLineWithRect:(CGRect)frame
{
    ...
    return [horizontalView autorelease];
}
