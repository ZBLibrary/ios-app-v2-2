...
- (void)layoutItemRects
{
    ...
    void(^contentUpdater)(CGRect)=NULL;
...
{            
    ...
    [newVisibleIndexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop)
    {
        ...
        if([self.dataSource respondsToSelector:@selector(listViewCurrentIndex:)])
        {
            [self.dataSource listViewCurrentIndex:(int)idx];
        }
        
        if([self.dataSource respondsToSelector:@selector(listViewCurrentIndexAndRect:rect:view: point:)])
        {
            [self.dataSource listViewCurrentIndexAndRect:(int)idx rect:itemRect view:view point:self.contentOffset];
        }
...
- (void)reloadData
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(overReloadListView:)])
    {
        [self.delegate overReloadListView:self];
    }
...
{    
    CGSize(^sizeForItemAtIndex)(NSUInteger)=NULL;
...
{
    NSLog(@"scrollOffset=%f\n",scrollView.contentOffset.x);
}

- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView
{
    NSLog(@"22222222scrollOffset=%f\n",scrollView.contentOffset.x);
}
...
- (void)layoutSubviews
{
    ...
    if([self.dataSource respondsToSelector:@selector(listViewOffset:)])
    {
        [self.dataSource listViewOffset:self.contentOffset.x];
    }
}
