...
@property (nonatomic, assign) BOOL itemViewTapFlag1;            //tap begin to carouselDidEndScrollingAnimation the itemViewTapFlag1 is yes , other no
@property (nonatomic, assign) BOOL itemViewTapFlag2;            //tap begin to (carouselDidEndScrollingAnimation && _toggle!=0.) the itemViewTapFlag2 is yes , other no
...
#ifdef COVERFLOW2_TAPEFFECT_SIMULATE_COVERFLOW
            if (_itemViewTapFlag1==NO && _toggle==0.) {
                _itemViewTapFlag2=NO;
            }
#endif
            
#ifdef COVERFLOW2_TAPEFFECT_SIMULATE_COVERFLOW
            if (_type == iCarouselTypeCoverFlow2  && _itemViewTapFlag2 == NO)
#else 
...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, iCarousel *self)
{
    ...
    if (difference == 0.0f)
    {
        ...
        difference = fabsf(x2 - x3) - fabs(x1 - x3);
    }
...
- (void)reloadItemAtIndex:(NSInteger)index animated:(BOOL)animated direction:(BOOL)direction
{
    //get container view
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gesture
{
    if ([gesture isKindOfClass:[UIPanGestureRecognizer class]])
    {
        ...
        if (_ignorePerpendicularSwipes)
        {
            ...
            {
                return fabsf(translation.x) >= fabs(translation.y);
            }
        }
    }
    return YES;
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    {
        _itemViewTapFlag1=YES;
        _itemViewTapFlag2=YES;
